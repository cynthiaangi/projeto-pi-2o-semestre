...
const codigosCidade = ["", 0, 3500105, 3500204, 3500303, 3500402, 3500501, 3500550, 3500600, 3500709, 3500758, 3500808, 3500907, 3501004, 3501103, 3501152, 3501202, 3501301, 3501400,
...
let myChart3 = new Chart(ctz, {
  ...
  data: {
    labels: [
      "Acima da meta ( > 95%)",
      "Abaixo sem risco (< 95% e > 85%)",
      "Risco epidemiológico (< 85%)",
    ],
    datasets: [
      {
        data: dados3,
...
const help1 =
  "Exibe a divisão percentual entre pessoas vacinadas e não vacinadas, apresentando um comparativo direto da cobertura vacinal na população.";
const titulo1 = "Total de vacinados e não vacinados";
const help2 =
  "Mostra o percentual de aumento ou redução da cobertura vacinal em relação a um período anterior, indicando a tendência de adesão à vacina.";
const titulo2 = "Variação da cobertura vacinal";
const help3 =
  "Apresenta a mudança percentual no número de casos registrados da doença em relação ao período anterior, indicando aumento ou redução.";
...
function abrirMenuExtendido() {
  ...
  var lateralExtendido =
    document.getElementsByClassName("lateral-extendido")[0];
...
async function criarGraficoSituacaoCobertura(idDoenca) {
  try {
    var respostaAcima = await fetch(
      `/medidas/criarGraficoSituacao95Cobertura/${idDoenca}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
    ...
    var respostaBaixo = await fetch(
      `/medidas/criarGraficoSituacao85Cobertura/${idDoenca}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
